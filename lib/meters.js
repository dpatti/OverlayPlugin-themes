  ...
  _createClass(App, [{
    ...
    value: function onOverlayDataUpdate(e) {
      ...
      if (isActive(this.state.currentEncounter)) {
        accumulator = this.updateWithAccumulator(this.state.currentEncounter, currentEncounter);
      } // Encounter ended
    ...
    key: "updateWithAccumulator",
    value: function updateWithAccumulator(prev, next) {
      var _prev$Encounter$elaps;

      var acc = this.state.accumulator;
      var now = performance.now();
      var elapsed = prev.Encounter.DURATION === next.Encounter.DURATION ? 0 : now - acc.lastUpdate; // Uptime: for each player, we keep track of some extra data, like when the
      ...
      var GCD = 2500; // milliseconds

      next.Encounter.elapsed = ((_prev$Encounter$elaps = prev.Encounter.elapsed) !== null && _prev$Encounter$elaps !== void 0 ? _prev$Encounter$elaps : 0) + elapsed;

      var actions = function actions(c) {
        return c ? parseInt(c.hits) + parseInt(c.heals) : 0;
      };

      for (var name in next.Combatant) {
        var _prev$Combatant$name$, _prev$Combatant$name$2;

        var prevLastAction = (_prev$Combatant$name$ = prev.Combatant[name].lastAction) !== null && _prev$Combatant$name$ !== void 0 ? _prev$Combatant$name$ : 0;
        var prevUptime = (_prev$Combatant$name$2 = prev.Combatant[name].uptime) !== null && _prev$Combatant$name$2 !== void 0 ? _prev$Combatant$name$2 : 0;
        var uptime = prevUptime + (now - prevLastAction < GCD ? elapsed : // Add any possible partial update: `acc.lastUpdate - prevLastAction`
        // represents the time we've already credited to this player's
        // uptime
        Math.max(GCD - (acc.lastUpdate - prevLastAction), 0));
        var didAction = actions(prev.Combatant[name]) < actions(next.Combatant[name]);

        _.extend(next.Combatant[name], _defineProperty({
