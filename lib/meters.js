  ...
  _createClass(App, [{
    ...
    value: function componentDidMount() {
      ...
      try {
        ...
        var playerName = (_localStorage$getItem = localStorage.getItem(App.PLAYER_NAME_KEY)) !== null && _localStorage$getItem !== void 0 ? _localStorage$getItem : options.you;
    ...
    value: function onOverlayDataUpdate(e) {
      ...
      if (!isActive(this.state.currentEncounter) && isActive(currentEncounter)) {
        var lastUpdateDelta = performance.now() - this.state.lastLogLine;
        this.setState({
      ...
      this.embellish(currentEncounter); // Encounter ended
    ...
    value: function onLogLine(e) {
      ...
      if (((_this$state$currentEn = this.state.currentEncounter) === null || _this$state$currentEn === void 0 ? void 0 : _this$state$currentEn.isActive) !== "true") {
        this.setState({
      ...
      if (code === "02") {
        ...
        var _getRecord = getRecord(_sourceName2),
            ...
            uptime = _getRecord.uptime; // If you were casting, you get credit for the duration of that cast or
    ...
    key: "embellish",
    value: function embellish(data) {
      var _this$state = this.state,
          serverTime = _this$state.serverTime,
          playerName = _this$state.playerName,
          actionTracking = _this$state.actionTracking,
          encounterStart = _this$state.encounterStart;
      ...
      var encounterDuration = serverTime - encounterStart; // This is different from the encounter's notion of duration because ACT
      ...
      for (var name in data.Combatant) {
        var _actionTracking$name$, _actionTracking$name;

        var uptime = (_actionTracking$name$ = (_actionTracking$name = actionTracking[name]) === null || _actionTracking$name === void 0 ? void 0 : _actionTracking$name.uptime) !== null && _actionTracking$name$ !== void 0 ? _actionTracking$name$ : 0; // We're using ACT's notion of encounter here, which might trim downtime,
